...
public extension UIView {
    ...
    fileprivate func showToast(toast: UIView, duration: Double, position: AnyObject) {
        ...
        if existToast != nil {
            if let timer: Timer = objc_getAssociatedObject(existToast!, &RMLoaderTimer) as? Timer {
                timer.invalidate()
            }
    ...
    fileprivate func makeToastActivity(position pos: AnyObject, message msg: String = "",img: UIImage = #imageLiteral(resourceName: "qr-code")) // image check for
    {
        ...
        activityIndicatorView.frame = CGRect(x: 0, y: 0, width: 160, height: 75)
        ...
        let bgView = UIView()
        bgView.frame = CGRect(x: 0, y: 0, width: (VGConstants.APP_DEL.window?.frame.width)!, height: (VGConstants.APP_DEL.window?.frame.height)!)
        bgView.backgroundColor = UIColor.black.withAlphaComponent(0.4)
        addSubview(bgView)
        
        if img != #imageLiteral(resourceName: "qr-code")
        {
            let activityIndicatorView = UIImageView()
            activityIndicatorView.image = img
            activityIndicatorView.frame = CGRect(x: 20, y: activityView.frame.origin.y + 100, width: (VGConstants.APP_DEL.window?.frame.width)! - 40 , height: (VGConstants .APP_DEL.window?.frame.width)! / 2)
            //activityIndicatorView.center = CGPoint(x: activityView.bounds.size.width / 2, y: activityView.bounds.size.height / 2)
            self.isUserInteractionEnabled = false;
